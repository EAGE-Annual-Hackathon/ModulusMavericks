...
def fast_gaussian(dimension, Sdev, Corr):
    ...
    if len(dimension) == 1:
        ...
        raise ValueError("FastGaussian: Wrong input, dimension should have length at most 2")
...
def gaussian_with_variable_parameters(field_dim, mean_value, sdev, mean_corr_length, std_corr_length):
    """
    ...
    if len(field_dim) < 3:
        ...
        if np.isscalar(sdev):
            for i in range(field_dim[2]):
                ...
                x[idx_range] = mean_value[idx_range] + fast_gaussian(field_dim[:2], sdev, corr_length)
                # Generate new correlation length for the next layer
                corr_length = add_gnoise(mean_corr_length, std_corr_length, 1)
        else:
            for i in range(field_dim[2]):
                idx_range = slice(i * layer_dim, (i + 1) * layer_dim)
                x[idx_range] = mean_value[idx_range] + fast_gaussian(field_dim[:2], sdev[idx_range], corr_length)
                # Generate new correlation length for the next layer
                corr_length = add_gnoise(mean_corr_length, std_corr_length, 1)
...
def adjust_variable_within_bounds(variable, lowerbound=None, upperbound=None):
    ...
    if lowerbound is not None:
        if np.isscalar(lowerbound):
            ...
            variable[variable < lowerbound_repeated] = lowerbound_repeated[variable < lowerbound_repeated]
    ...
    if upperbound is not None:
        if np.isscalar(upperbound):
            ...
            variable[variable > upperbound_repeated] = upperbound_repeated[variable > upperbound_repeated]
...
def NorneGeostat():
    ...
    for nr in range(dim[2] - 1):
        ...
        values1 = np.concatenate((k[values1_range_start:values1_range_end], 
        ...
        values2 = np.concatenate((k[values2_range_start:values2_range_end], 
...
def Plot_RSM_percentile(pertoutt,True_mat,timezz):
    ...
    columns = ['B_1BH','B_1H','B_2H','B_3H','B_4BH',
               'B_4DH','B_4H','D_1CH','D_1H','D_2H','D_3AH',
               'D_3BH','D_4AH','D_4H','E_1H',
               'E_2AH','E_2H','E_3AH','E_3CH','E_3H','E_4AH','K_3H']    
    ...
    for k in range(22):
        ...
        plt.plot(timezz,True_mat[:,k], color = 'red', lw = '2',\
                 label ='Flow')
        plt.plot(timezz,P10[:,k], color = 'blue', lw = '2', \
                 label ='Modulus')        
    ...
    #os.chdir('RESULTS') 
    plt.suptitle('NORNE - Oil Production ($Q_{oil}(bbl/day)$)', fontsize=16, fontweight='bold')    
    plt.savefig('Oil.png')          # save as png                                  # preventing the figures from showing
    #os.chdir(oldfolder)
    plt.clf()
    plt.close()
    ...
    for k in range(22):
        plt.subplot(5,5,int(k+1))
        plt.plot(timezz,True_mat[:,k+22], color = 'red', lw = '2',\
                 label ='Flow')
        plt.plot(timezz,P10[:,k+22], color = 'blue', lw = '2', \
                 label ='Modulus')        
        plt.xlabel('Time (days)',fontsize = 13,fontweight='bold')
    ...
    #os.chdir('RESULTS') 
    plt.suptitle('NORNE - Water Production ($Q_{water}(bbl/day)$)', fontsize=16, fontweight='bold')    
    plt.savefig('Water.png')          # save as png                                  # preventing the figures from showing
    #os.chdir(oldfolder)
    plt.clf()
    plt.close()
    ...
    for k in range(22):
        plt.subplot(5,5,int(k+1))
        plt.plot(timezz,True_mat[:,k+44], color = 'red', lw = '2',\
                 label ='Flow')
        plt.plot(timezz,P10[:,k+44], color = 'blue', lw = '2', \
                 label ='Modulus')        
        plt.xlabel('Time (days)',fontsize = 13,fontweight='bold')
    ...
    #os.chdir('RESULTS')
    plt.suptitle('NORNE - Gas Production ($Q_{gas}(scf/day)$)', fontsize=16, fontweight='bold') 
     
    plt.savefig('Gas.png')          # save as png                                  # preventing the figures from showing
    #os.chdir(oldfolder)
    plt.clf()
    plt.close()                   
...
def Plot_2D(XX,YY,plt,nx,ny,nz,Truee,N_injw,N_pr,N_injg,varii,injectors,producers,gass):
...
def Plot3DNorne(nx,ny,nz,Truee,N_injw,N_pr,N_injg,cgrid,varii,injectors,producers,gass):
    ...
    for mm in range(n_inj):
        ...
        ax.text(x_line_end, y_line_end, z_line_end, discrip, color='blue',weight='bold', fontsize=5)
    ...
    for mm in range(N_injg):
        ...
        ax.text(x_line_end, y_line_end, z_line_end, discrip, color='red',weight='bold', fontsize=5)
    ...
    for mm in range(n_prod):
        ...
        ax.text(x_line_end, y_line_end, z_line_end, discrip, color='g',weight='bold', fontsize=5) 
    ...
    green_line = mlines.Line2D([], [], color='green', linewidth=2, label='oil/water/gas Producer')
    ...
    if varii =='perm':
        ...
        ax.set_title('Permeability Field with well locations',fontsize=16,weight='bold')
    elif varii =='water':
        cbar.set_label('water saturation', fontsize=9)
        ax.set_title('water saturation Field with well locations',fontsize=16,weight='bold')
    elif varii =='oil':
        cbar.set_label('Oil saturation', fontsize=9) 
        ax.set_title('Oil saturation Field with well locations',fontsize=16,weight='bold')
    elif varii =='porosity':
    ...
    for j in range(nz):
        ...
        h1 = ax1.plot_surface(Xcor2D, Ycor2D, Zcor2D,
                              cmap='jet', facecolors=colors,
    ...
    for mm in range(n_inj):
        ...
        ax1.text(x_line_end, y_line_end, z_line_end, discrip, color='black',weight='bold', fontsize=16)
    ...
    for mm in range(n_prod):
        ...
        ax1.text(x_line_end, y_line_end, z_line_end, discrip, color='r',weight='bold', fontsize=16)   
    ...
    if varii =='perm':
        fig.suptitle(r'3D Permeability NORNE FIELD [$N_x = 46$, $N_y = 112$, $N_z = 22$]',weight='bold', fontsize=15) 
    elif varii =='water':
        fig.suptitle(r'3D Water Saturation NORNE FIELD [$N_x = 46$, $N_y = 112$, $N_z = 22$]',weight='bold', fontsize=15)
    elif varii =='oil':
        fig.suptitle(r'3D Oil Saturation NORNE FIELD [$N_x = 46$, $N_y = 112$, $N_z = 22$]',weight='bold', fontsize=15)
    elif varii =='porosity':
        fig.suptitle(r'3D Porosity NORNE FIELD [$N_x = 46$, $N_y = 112$, $N_z = 22$]',weight='bold', fontsize=15)
...
def Plot_all_layesr(nx,ny,nz,see,injectors,producers,gass,varii):
    ...
    for i in range(nz):
        ...
        if varii =='perm':
            cbar.ax.set_ylabel('Log K(mD)', fontsize=9)
            plt.title('Permeability Field Layer_' + str(i+1), fontsize=11,weight='bold')
        if varii =='porosity':
            cbar.ax.set_ylabel('porosity', fontsize=9)
            plt.title('porosity Field Layer_' + str(i+1),weight='bold',fontsize=11)
        if varii =='oil':
            cbar.ax.set_ylabel('Oil saturation', fontsize=9)
            plt.title('Oil saturation Field Layer_' + str(i+1),weight='bold',fontsize=11)
        if varii =='water':
            cbar.ax.set_ylabel('Water saturation', fontsize=9)
            plt.title('Water saturation Field Layer_' + str(i+1),weight='bold',fontsize=11)     
    ...
    if varii =='perm':
        plt.suptitle(r'Permeability NORNE FIELD [$N_x = 46$, $N_y = 112$, $N_z = 22$]',weight='bold', fontsize=15) 
    elif varii =='water':
        plt.suptitle(r'Water Saturation NORNE FIELD [$N_x = 46$, $N_y = 112$, $N_z = 22$]',weight='bold', fontsize=15)
    elif varii =='oil':
        plt.suptitle(r'Oil Saturation NORNE FIELD [$N_x = 46$, $N_y = 112$, $N_z = 22$]',weight='bold', fontsize=15)
    elif varii =='porosity':
        plt.suptitle(r'Porosity NORNE FIELD [$N_x = 46$, $N_y = 112$, $N_z = 22$]',weight='bold', fontsize=15)
...
def Add_marker2(plt, XX, YY, injectors,producers,gass):
    ...
    for mm in range(n_inj):
        ...
        plt.scatter(XX.T[xloc-1, yloc-1]+0.5, YY.T[xloc-1, yloc-1]+0.5,
                    s=200, marker='v', color='white')         
        plt.text(XX.T[xloc-1, yloc-1]+0.5, YY.T[xloc-1, yloc-1]+0.5, discrip, color='black',weight='bold',
...
def ensemble_pytorch(param_perm,param_poro,param_fault,\
                     nx,ny,nz,Ne,effective,oldfolder,target_min,target_max,minK,maxK,
    ...
    inn = {'perm': torch.from_numpy(ini_ensemble1).to(device,torch.float32),\
...
def Geta_all(folder,nx,ny,nz,effective,oldfolder,check,steppi,steppi_indices):
    ...
    for pr_slice, sw_slice, sg_slice in zip(filtered_pressure, 
                    filtered_swat, filtered_sgas):
...
def Get_RSM_FLOW(oldfolder,N,steppi,steppi_indices): 
    ...
    for i in range(N):  
        ...
        A222oilsim = pd.read_csv('FULLNORNE2.RSM', skiprows=2057, sep='\s+', header=None)
        ...
        ##IMPORT FOR WATER
        A2watersim = pd.read_csv('FULLNORNE2.RSM', skiprows=2313, sep='\s+', header=None)
        B_1BHwatersim = A2watersim[9].values[:steppi]
    
        A22watersim = pd.read_csv('FULLNORNE2.RSM', skiprows=2569, sep='\s+', header=None)
        ...
        A222watersim = pd.read_csv('FULLNORNE2.RSM', skiprows=2825, sep='\s+', header=None)
        ...
        A222watersim = pd.read_csv('FULLNORNE2.RSM', skiprows=3081, sep='\s+', header=None)
        ...
        A222gassim = pd.read_csv('FULLNORNE2.RSM', skiprows=1545, sep='\s+', header=None)
    ...
    OIL = {"B_1BH": WOIL1,"B_1H": WOIL2,"B_2H": WOIL3,"B_3H": WOIL4,"B_4BH": WOIL5,"B_4DH": WOIL6,\
    "B_4H": WOIL7,"D_1CH": WOIL8,"D_1H": WOIL9,"D_2H": WOIL10,"D_3AH": WOIL11,"D_3BH": WOIL12,\
    "D_4AH": WOIL13,"D_4H": WOIL14,"E_1H": WOIL15,"E_2AH": WOIL16,"E_2H": WOIL17,"E_3AH": WOIL18,\
    "E_3CH": WOIL19,"E_3H": WOIL20,"E_4AH": WOIL21,"K_3H": WOIL22}  
        
    OIL = {k: abs(v) for k, v in OIL.items()}
        

    WATER = {"B_1BH": WWATER1,"B_1H": WWATER2,"B_2H": WWATER3,"B_3H": WWATER4,"B_4BH": WWATER5,"B_4DH": WWATER6,\
    "B_4H": WWATER7,"D_1CH": WWATER8,"D_1H": WWATER9,"D_2H": WWATER10,"D_3AH": WWATER11,"D_3BH": WWATER12,\
    "D_4AH": WWATER13,"D_4H": WWATER14,"E_1H": WWATER15,"E_2AH": WWATER16,"E_2H": WWATER17,"E_3AH": WWATER18,\
    "E_3CH": WWATER19,"E_3H": WWATER20,"E_4AH": WWATER21,"K_3H": WWATER22} 
    
    WATER = {k: abs(v) for k, v in WATER.items()}

        
    GAS = {"B_1BH": WGAS1,"B_1H": WGAS2,"B_2H": WGAS3,"B_3H": WGAS4,"B_4BH": WGAS5,"B_4DH": WGAS6,\
    "B_4H": WGAS7,"D_1CH": WGAS8,"D_1H": WGAS9,"D_2H": WGAS10,"D_3AH": WGAS11,"D_3BH": WGAS12,\
    "D_4AH": WGAS13,"D_4H": WGAS14,"E_1H": WGAS15,"E_2AH": WGAS16,"E_2H": WGAS17,"E_3AH": WGAS18,\
    "E_3CH": WGAS19,"E_3H": WGAS20,"E_4AH": WGAS21,"K_3H": WGAS22}

    GAS = {k: abs(v) for k, v in GAS.items()}         

    WATERINJ = {"C_1H": WWINJ1,"C_2H": WWINJ2,"C_3H": WWINJ3,"C_4AH": WWINJ4,"C_4H": WWINJ5,"F_1H": WWINJ6,\
                "F_2H": WWINJ7,"F_3H": WWINJ8,"F_4H": WWINJ9} 
...
def Get_predicted_data(OIL,WATER,GAS,WATERINJ,GASINJ):
    ...
    predicted_data = np.nan_to_num(np.vstack((result_oil, result_water,result_gas,result_winj,result_ginj)))
...
def save_files(perm,poro,perm2,dest_dir,oldfolder): 
    ...
    for i, line in enumerate(lines):
        if line.strip() == 'MULTFLT':
            ...
            parts = line.split(' ')
            if len(parts) > 1 and parts[1].replace('.', '', 1).replace('/', '').isdigit():
...
def Get_data_FFNN(oldfolder,N,pressure,Sgas,Swater,Soil,perm,Time,steppi,steppi_indices): 
    ...
    for i in range(N):          
        ...
        permxx = np.hstack((a1,a2,a3,a5,a6,a7,a8,a9,a10,a11,a12,a13,a14,a15,a16,a17,a18,a19,a20,a21,a22,a23))
...
def Get_data_FFNN1(folder,oldfolder,N,pressure,Sgas,Swater,Soil,perm,Time,steppi,steppi_indices): 
...
def historydata(timestep,steppi,steppi_indices):    
   ...
   # A1gin = np.genfromtxt('NORNE_ATW2013.RSM', dtype=float, usecols=(1, 3, 4, 5, 6, 8), skip_header=73977)
    ...
    DATA = {"OIL": WOIL1,"WATER": WWATER1,"GAS": WGAS1,"WATER_INJ": WWINJ1,"WGAS_inj": WGASJ1} 
...
def Plot_Modulus(ax,nx,ny,nz,Truee,N_injw,N_pr,N_injg,varii,injectors,producers,gass):
    #matplotlib.use('Agg')
    Pressz = np.reshape(Truee,(nx,ny,nz),'F')
    ...
    if varii =='perm':
        ...
        ax.set_title('Permeability Field with well locations',fontsize=12,weight='bold')
    elif varii =='water Modulus':
        ...
        ax.set_title('water saturation - (Numerical(Flow) -Modulus))',fontsize=12,weight='bold')        
        ...
        cbar.set_label('unit', fontsize=12)
        ax.set_title('oil saturation - (Numerical(Flow) -Modulus))',fontsize=12,weight='bold') 
        ...
        cbar.set_label('unit', fontsize=12)
        ax.set_title('gas saturation - (Numerical(Flow) -Modulus))',fontsize=12,weight='bold') 
        ...
        cbar.set_label('unit', fontsize=12)
        ax.set_title('Pressure - (Numerical(Flow) -Modulus))',fontsize=12,weight='bold') 
...
def load_FNO_dataset2(path, input_keys, output_keys, n_examples=None):
    "Loads a FNO dataset"
...
class Black_oil(torch.nn.Module):
    ...
    def __init__(self,neededM,SWI,SWR,UW,BW,UO,BO,
                 ...
                 minP,maxP,p_bub,p_atm,CFO,Relperm,params,pde_method,RE,max_inn_fcn,max_out_fcn,DZ,device):
        super().__init__()
    ...
    def forward(self, input_var: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:       
        ...
        dtime = input_var["dt"]
        #pini = input_var["Pini"]
        pini = input_var["Pbefore"]
        poro = input_var["Phi"]
        #sini = input_var["Swini"]
        sini = input_var["Swbefore"]
        #sat = input_var["water_sat"]
        sat = input_var["Swafter"]
        satg = input_var["Sgafter"]
        Sgbefore = input_var["Sgbefore"]
        satg = input_var["Sgafter"]
        ...
        UG = calc_mu_g(avg_p)
        RS = calc_rs(self.p_bub, avg_p, device)
        ...
        dp_darcy = dp - (u-pini)
        dp_water = dsww - (sat - sini)
        dp_gas = dsgg - (satg - Sgbefore)        
        ...
        output_var = {"pressured": p_loss,"saturationd": s_loss,"saturationdg": sg_loss,"dpg":dp_darcy,
        "dp_waterg":dp_water,"dp_gasg":dp_gas}
...
def run(cfg: ModulusConfig) -> None:
    ...
    if DEFAULT ==1:
        ...
        fno_typee = None
        while True:
            fno_typee = int(input('Select 1 = Use peacemann and darcy loss  | 2 = use peacemann loss \n'))
    ...
    SWOW = torch.tensor(np.array(np.vstack(cfg.custom.WELLSPECS.SWOW),dtype=float)).to(manager.device)
    
    SWOG = torch.tensor(np.array(np.vstack(cfg.custom.WELLSPECS.SWOG),dtype=float)).to(manager.device)
    ...
    DZ = torch.tensor(100).to(manager.device)
    RE = torch.tensor(0.2 * 100).to(manager.device)    
    ...
    if interest == 1:    
        ...
        for i in range(N):
            folder = to_absolute_path('../RUNS/Realisation' + str(i))
            Pr,sw,sg,so,tt,flt = Geta_all(folder,nx,ny,nz,effective,oldfolder,check,steppi,steppi_indices)
        ...
        inn_fcn,out_fcn = Get_data_FFNN(oldfolder,N,pressure,Sgas,Swater,Soil,
                                permeability,Time,steppi,steppi_indices)
    ...
    SWI = torch.from_numpy(np.array(SWI)).to(manager.device)
    SWR = torch.from_numpy(np.array(SWR)).to(manager.device)
    UW = torch.from_numpy(np.array(UW)).to(manager.device)
    BW = torch.from_numpy(np.array(BW)).to(manager.device)
    UO = torch.from_numpy(np.array(UO)).to(manager.device)
    BO = torch.from_numpy(np.array(BO)).to(manager.device)
    
    # SWOW = torch.from_numpy(SWOW).to(device)
    # SWOG = torch.from_numpy(SWOG).to(device)
    p_bub = torch.from_numpy(np.array(p_bub)).to(manager.device)
    p_atm = torch.from_numpy(np.array(p_atm)).to(manager.device)
    CFO = torch.from_numpy(np.array(CFO)).to(manager.device)    
    ...
    minKx = torch.from_numpy(minK).to(manager.device)
    maxKx = torch.from_numpy(maxK).to(manager.device)
    minTx = torch.from_numpy(minT).to(manager.device)
    maxTx = torch.from_numpy(maxT).to(manager.device)
    minPx = torch.from_numpy(minP).to(manager.device)
    maxPx = torch.from_numpy(maxP).to(manager.device)
    minQx = torch.from_numpy(minQ).to(manager.device)
    maxQx = torch.from_numpy(maxQ).to(manager.device)
    minQgx = torch.from_numpy(minQg).to(manager.device)
    maxQgx = torch.from_numpy(maxQg).to(manager.device)
    minQwx = torch.from_numpy(minQw).to(manager.device)
    maxQwx = torch.from_numpy(maxQw).to(manager.device)
    min_inn_fcnx = torch.from_numpy(min_inn_fcn).to(manager.device)
    max_inn_fcnx = torch.from_numpy(max_inn_fcn).to(manager.device)
    min_out_fcnx = torch.from_numpy(min_out_fcn).to(manager.device)
    max_out_fcnx = torch.from_numpy(max_out_fcn).to(manager.device)  
    ...
    neededM = {'Q': torch.from_numpy(cQ).to(manager.device,torch.float32),\
    'Qw': torch.from_numpy(cQw).to(manager.device,dtype=torch.float32),\
    'Qg': torch.from_numpy(cQg).to(manager.device,dtype=torch.float32),\
     'actnum': torch.from_numpy(cactnum).to(manager.device,dtype=torch.float32),\
     'Time': torch.from_numpy(cTime).to(manager.device,dtype=torch.float32)} 
    ...
    for kk in range(N_ens):        
        ...
        for mum in range(steppi):
            for i in range(nz):
                ...
                cSato[kk,mum,i,:,:] = clip_and_convert_to_float3(X_data1['Oil_saturation'][kk,mum,:,:,i])
    ...
    for kk in range(N_ens):        
        #INPUTS 
        cPress_bb = np.zeros((steppi,nz,nx,ny),dtype=np.float32)
        cSat_bb = np.zeros((steppi,nz,nx,ny),dtype=np.float32)
        cSatg_bb =  np.zeros((steppi,nz,nx,ny),dtype=np.float32)

        cQ_eage = np.zeros((steppi,nz,nx,ny),dtype=np.float32)
        cQw_eage = np.zeros((steppi,nz,nx,ny),dtype=np.float32)
        cQg_eage = np.zeros((steppi,nz,nx,ny),dtype=np.float32)

        cPress_aa = np.zeros((steppi,nz,nx,ny),dtype=np.float32)
        cSat_aa = np.zeros((steppi,nz,nx,ny),dtype=np.float32)
        cSatg_aa =  np.zeros((steppi,nz,nx,ny),dtype=np.float32)

        cTimeeage = np.zeros((steppi,nz,nx,ny),dtype=np.float32)
        cTimeafter = np.zeros((steppi,nz,nx,ny),dtype=np.float32)

        for t in range(steppi): 
            cPermi = cPerm[kk,0,:,:,:][None,None,:,:,:]
            cPermi_2.append(cPermi)
            cPhii = cPhi[kk,0,:,:,:][None,None,:,:,:]
            cPhii_2.append(cPhii)
            cfaulti = cfault[kk,0,:,:,:][None,None,:,:,:]
            cfault_2.append(cfaulti)

            if t == 0:
                cPress_b = cPini[kk,0,:,:,:]/maxP
                cSat_b = cSini[kk,0,:,:,:]
                cSatg_b = np.zeros((nz,nx,ny),dtype=np.float32)
            else:
                cPress_b = cPress[kk,t-1,:,:,:]
                cSat_b = cSat[kk,t-1,:,:,:]
                cSatg_b = cSatg[kk,t-1,:,:,:]

            cPress_bb[t,:,:,:]=cPress_b
            cSat_bb[t,:,:,:] = cSat_b
            cSatg_bb[t,:,:,:] = cSatg_b

            CTimee = cTime[0, t, :, :, :]  # Initialize CTimee here
            cTimeeage[t, :, :, :] = CTimee
            if t == 0:
                cTimeea = np.zeros((1, nz, nx, ny),dtype=np.float32)
            else:
                cTimeea = CTimee

            cTimeafter[t, :, :, :] = cTimeea


            cPress_aa[t,:,:,:]=cPress[kk,t,:,:,:]
            cSat_aa[t,:,:,:] = cSat[kk,t,:,:,:]
            cSatg_aa[t,:,:,:] = cSatg[kk,t,:,:,:]   
        ...
        dt = abs(cTimeafter - cTimeeage)
        
        cPress_bb = cPress_bb[:,None,:,:,:]
        cSat_bb = cSat_bb[:,None,:,:,:]
        cSatg_bb = cSatg_bb[:,None,:,:,:]

        cPress_aa = cPress_aa[:,None,:,:,:]
        cSat_aa = cSat_aa[:,None,:,:,:]
        cSatg_aa = cSatg_aa[:,None,:,:,:]


        cPress_before.append(cPress_bb)
        cSat_before.append(cSat_bb)
        cSatg_before.append(cSatg_bb)

        cPress_after.append(cPress_aa)
        cSat_after.append(cSat_aa)
        cSatg_after.append(cSatg_aa)

        cQ_in_2.append(cQ_eage)
        cQw_in_2.append(cQw_eage)
        cQg_in_2.append(cQg_eage)

        cTimeuse.append(cTimeeage)
        dtuse.append(dt)

    
    cPermi_2 = np.concatenate(cPermi_2, axis = 0)
    cPhii_2 = np.concatenate(cPhii_2, axis = 0)
    cfault_2 = np.concatenate(cfault_2, axis = 0)
    cPress_before = np.concatenate(cPress_before, axis = 0)
    cSat_before = np.concatenate(cSat_before, axis = 0)
    cSatg_before = np.concatenate(cSatg_before, axis = 0)
    cQg_in_2 = np.concatenate(cQg_in_2, axis = 0)
    cQg_in_2 = cQg_in_2[:,None,:,:,:]
    cQw_in_2 = np.concatenate(cQw_in_2, axis = 0)
    cQw_in_2 = cQw_in_2[:,None,:,:,:]
    cQ_in_2 = np.concatenate(cQ_in_2, axis = 0)
    cQ_in_2 = cQ_in_2[:,None,:,:,:]
    cTimeuse = np.concatenate(cTimeuse, axis = 0)
    cTimeuse = cTimeuse[:,None,:,:,:]
    dtuse = np.concatenate(dtuse, axis = 0)
    dtuse = dtuse[:,None,:,:,:]

    cPress_after = np.concatenate(cPress_after, axis = 0)
    cSat_after = np.concatenate(cSat_after, axis = 0)
    cSatg_after = np.concatenate(cSatg_after, axis = 0)

    dpeage = abs(cPress_after - cPress_before)
    dsweage = abs(cSat_after - cSat_before)
    dsgeage = abs(cSatg_after - cSatg_before)



    del X_data1
    gc.collect()
    data = {'perm': cPerm,'Phi': cPhi,
        'fault':cfault,'Pbefore':cPress_before,'Pafter':cPress_after,'Swbefore':cSat_before, 'Q':cQ_in_2, 'Qg':cQg_in_2, 'Qw': cQw_in_2,
        'Swafter':cSat_after,'Sgbefore':cSatg_before,'Sgafter':cSatg_after,'dp':dpeage,'dsww':dsweage,'dsgg':dsgeage, 'time':cTimeuse,
        'dt':dtuse}

    data_test = {'perm': cPerm,'Phi': cPhi,
        'fault':cfault,'Pbefore':cPress_before,'Pafter':cPress_after,'Swbefore':cSat_before, 'Q':cQ_in_2, 'Qg':cQg_in_2, 'Qw': cQw_in_2,
        'Swafter':cSat_after,'Sgbefore':cSatg_before,'Sgafter':cSatg_after,'dp':dpeage,'dsww':dsweage,'dsgg':dsgeage, 'time':cTimeuse,
        'dt':dtuse}
    ...
    for key in outvar_train.keys():
        outvar_train[key][np.isnan(outvar_train[key])] = 1e-6           # Convert NaN to 0
        outvar_train[key][np.isinf(outvar_train[key])] = 1e-6           # Convert infinity to 0 
    ...
    for key in outvar_test.keys():
        outvar_test[key][np.isnan(outvar_test[key])] = 1e-6           # Convert NaN to 0
        outvar_test[key][np.isinf(outvar_test[key])] = 1e-6           # Convert infinity to 0 
    ...
    if fno_type == 'PINO':
        outvar_train["pressured"] = np.zeros_like(
            outvar_train["Swafter"]
        )

        outvar_train["saturationd"] = np.zeros_like(
            outvar_train["Swafter"]
        )

        outvar_train["saturationdg"] = np.zeros_like(
            outvar_train["Swafter"]
        )

        outvar_train["dpg"] = np.zeros_like(
            outvar_train["Swafter"]
        )

        outvar_train["dp_waterg"] = np.zeros_like(
            outvar_train["Swafter"]
        )

        outvar_train["dp_gasg"] = np.zeros_like(
            outvar_train["Swafter"]
        )
    ...
    decoder = ConvFullyConnectedArch([Key("z", size=32)], \
                [Key("Pafter", size=1),Key("Swafter", size=1),Key("Sgafter", size=1),
                Key("dp", size=1),Key("dsww", size=1),Key("dsgg", size=1)])

    fno = FNOArch([Key("perm", size=1),Key("Phi", size=1), Key("fault", size=1),\
                Key("Pbefore", size=1),Key("Swbefore", size=1),\
                Key("Sgbefore", size=1),Key("Q", size=1),\
                Key("Qg", size=1),Key("Qw", size=1),\
                Key("time", size=1),Key("dt", size=1) ], dimension=3, decoder_net=decoder) 
    ...
    if fno_type =='PINO':
        ...
        darcyy = Node(
            ...
            evaluate = Black_oil(neededM,SWI,SWR,UW,BW,UO,BO,
                          ...
                          minPx,maxPx,p_bub,p_atm,CFO,Relperm,params,pde_method,
                          RE,max_inn_fcnx,max_out_fcnx,DZ,manager.device),
            name="Darcy node",
        )
    

    if fno_type =='PINO':
        nodes = [fno.make_node('fno_forward_model')] +\
                [darcyy]
